...
static esp_err_t command(uint8_t value) { return esp_lcd_panel_io_tx_param(io, value, NULL, 0); }
static esp_err_t data(uint8_t value) { return esp_lcd_panel_io_tx_param(io, -1, &value, 1); }
...
esp_err_t glance_deck_rlcd_init(void) {
    ...
    esp_lcd_panel_io_spi_config_t spi = { .cs_gpio_num = RLCD_CS_GPIO, .dc_gpio_num = RLCD_DC_GPIO, .pclk_hz = 10000000, .lcd_cmd_bits = 8, .lcd_param_bits = 8, .spi_mode = 0, .trans_queue_depth = 1 };
    ESP_RETURN_ON_ERROR(esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)SPI3_HOST, &spi, &io), "rlcd", "panel IO init");
    gpio_config_t reset = { .pin_bit_mask = 1ULL << RLCD_RESET_GPIO, .mode = GPIO_MODE_OUTPUT, .pull_up_en = GPIO_PULLUP_ENABLE, .pull_down_en = GPIO_PULLDOWN_DISABLE, .intr_type = GPIO_INTR_DISABLE };
    ESP_RETURN_ON_ERROR(gpio_config(&reset), "rlcd", "reset GPIO init");
    ...
    // The controller-specific initialization follows Waveshare's official `display_bsp.cpp`.
    const uint8_t setup[][2] = {{0xD6,0x17},{0xD1,0x01},{0xC0,0x11},{0xC0,0x04},{0xB2,0x02},{0xB7,0x13},{0xB0,0x64}};
    for (size_t index = 0; index < sizeof(setup)/sizeof(setup[0]); index++) { ESP_RETURN_ON_ERROR(command(setup[index][0]), "rlcd", "setup command"); ESP_RETURN_ON_ERROR(data(setup[index][1]), "rlcd", "setup data"); }
...
esp_err_t glance_deck_rlcd_flush(const uint8_t *frame, size_t length) {
    ...
    ESP_RETURN_ON_ERROR(command(0x2C), "rlcd", "RAM write"); return esp_lcd_panel_io_tx_color(io, -1, frame, length);
}
