...
import { create_enrollment } from '@/server/enrollment'
import { current_administrator } from '@/server/session'

const enrollment_schema = z.object({ name: z.string().min(1).max(128), board_model: z.literal('ESP32-S3-RLCD-4.2') })
...
export async function POST(request: Request) {
  ...
  try { return NextResponse.json(await create_enrollment(body.data.name, body.data.board_model), { status: 201 }) }
