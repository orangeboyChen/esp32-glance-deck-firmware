...
export async function POST(request: Request) {
  ...
  const preview_svg = render_display_preview(body.data.document)
  const content_sha256 = createHash('sha256').update(preview_svg).digest('hex')
  const release = await db.transaction(async (transaction) => {
    ...
    const [created] = await transaction.insert(display_releases).values({ version: next_version, page_id: body.data.page_id, document: body.data.document, preview_svg, content_sha256 }).returning()
  ...
  const metadata = { id: release.created.id, version: release.created.version, page_id: release.created.page_id, image_sha256: content_sha256, image_bytes: Buffer.byteLength(preview_svg) }
